...
for f in EXPECTED_FNS:
    if not hasattr(wat, f):
        print(f, "is missing in wat.py.")
        wat.__dict__[f] = lambda *x: 0
...
class Clock(object):
    ...
    def __init__(self, tk, zone=None, **kargs):
        ...
        self.diff = wat.hours_difference(dlt, 0)
    ...
    def refresh(self, forced=False):
        ...
        hour = int(wat.time_from_utc(utc_hour, self.diff))
        ...
        minutes_angle = wat.get_minutes(int(self.now)) / 60 * 360
        if wat.time_from_utc(0, self.diff) % 1 == 0.5:
            minutes_angle += 180
        elif wat.time_from_utc(0, self.diff) % 1 == 0.75:
            minutes_angle += 270
        ...
        seconds_angle = int(wat.get_seconds(int(self.now))) / 60 * 360
